...
let inertPolyfillState: boolean | null = null;
async function ensureInertSupport(): Promise<boolean> {
    if (inertPolyfillState !== null) return inertPolyfillState;
    if ('inert' in HTMLElement.prototype) {
        inertPolyfillState = true;
        return true;
    }
    try {
        // @ts-expect-error - no types for wicg-inert
        await import('wicg-inert');
        inertPolyfillState = true;
        return true;
    } catch {
        inertPolyfillState = false;
        return false;
    }
}
 ...
 *   • body scroll lock
 *   • inert on body siblings (with polyfill + aria-hidden fallback)
 *   • focus trap inside menu
 *   • Escape to close (handled by caller via setter; we only call setOpenFalse)
...
export function useMobileMenuA11y(
    ...
    useEffect(() => {
        ...
        const applyInert = async (apply: boolean) => {
            await ensureInertSupport();
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                try {
                    if ('inert' in child) {
                        (child as HTMLElement & { inert?: boolean }).inert = apply;
                    } else if (apply) {
                    ...
                    if (apply) (child as HTMLElement).setAttribute('aria-hidden', 'true');
                    else (child as HTMLElement).removeAttribute('aria-hidden');
                }
            }
        };

        applyInert(open).catch(() => null);
        return () => {
            document.body.style.overflow = '';
            applyInert(false).catch(() => null);
        };
    }, [open, portalEl]);
