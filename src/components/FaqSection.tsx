                ...
                {copy.items.map((item, index) => (
                    <Disclosure as="div" key={`faq-${index}`} className="rounded-2xl border border-white/8 bg-studio-surface shadow-sm md:rounded-3xl">
                        {({ open }) => (
                            <>
                                <Disclosure.Button
                                    className="flex w-full items-start justify-between rounded-2xl px-5 py-4 text-left transition hover:bg-white/10 md:rounded-3xl md:px-6 md:py-5"
                                >
                                    <span className="text-base font-semibold text-white md:text-lg">{item.question}</span>
                                    <ChevronDownIcon
                                        className={`ml-4 h-5 w-5 flex-shrink-0 text-studio-accent transition-transform ${open ? 'rotate-180' : ''}`}
                                    />
                                </Disclosure.Button>
